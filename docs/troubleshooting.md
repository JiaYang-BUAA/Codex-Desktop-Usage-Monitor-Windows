...
文件应只包含白名单设置，不包含账号凭据。如果设置文件损坏，后台会回退到安全默认值：“7天剩余”和“当前任务累计 Token”。
...
测试包括协议解析、UI 生命周期、设置持久化、DPAPI、配置校验、启动器、安全扫描和发布包白名单。如果完整测试通过但实时界面异常，应继续检查动态 CDP 端口和当前 Codex DOM。
