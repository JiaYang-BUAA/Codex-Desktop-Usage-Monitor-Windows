...
直接在 Windows 版 Codex Desktop 输入区域旁查看官方订阅周期、任务 Token、社区重置预测、API 账户和 API Key 用量。监视器通过仅绑定本机的 Chrome DevTools Protocol（CDP）运行时注入，不是独立悬浮窗，也不修改 WindowsApps、`app.asar`、Codex 登录文件或模型配置。
...
- 同一面板分开展示本会话、官方订阅、可选的社区重置预测、API 账户和 API Key；可查看缓存命中率与近7天 Token。
...
点击监视栏可展开面板。新安装默认显示“本会话”“官方订阅”和“重置预测”；预测栏可在“官方订阅”栏底部的“设置”中关闭。需要 API 数据时可在同一设置区开启“API 栏”，恢复“API 账户”和“API Key”两栏；已有设置升级后保持原 API 布局。五栏同时显示时不会压窄单栏：面板会优先保证右边缘完整可见，并把增加的宽度向左延伸，左侧允许超出 Codex 内容区。“本会话”先显示当前会话累计 Token、最近一次已完成回答消耗 Token、缓存命中率和自动压缩上下文次数，最下面依次放置当前状态、额度恢复续跑及其发送内容；“官方订阅”增加近7天 Token。勾选额度恢复续跑后，折叠监视栏会直接出现续跑开关，和展开面板右侧开关控制当前任务的同一功能；极简模式只显示开关本体。数据项在折叠栏中严格按照勾选先后从左到右排列，取消后重新勾选会移到末尾。“5小时剩余”和“7天剩余”在展开面板中按“重置时间 · 剩余百分数”显示，折叠栏中仍只显示百分数。
...
显示项及其勾选顺序、API 栏和重置预测栏的显示状态、极简模式、倒计时可视化、English UI、自动更新，以及本会话栏中的额度恢复续跑开关和发送内容，由监视器后台保存到 `%LOCALAPPDATA%\CodexUsageMonitor\ui-settings.json`。六个全局选项默认收在“官方订阅”栏底部的“设置”入口中，并按两列三行排列。开启自动更新后，运行中的监视器最多每 24 小时检查一次本项目的 GitHub Release；发现正式新版本时，会下载名称与版本严格匹配的 Windows ZIP、核对 GitHub 提供的 SHA-256 摘要，校验通过后安装并热替换监视器后台，不会重启 Codex。检查、下载或校验失败时继续使用当前版本，下个周期再试。
...
| 重置预测 | 显示开源项目 [gussuri/codex-reset-observatory](https://github.com/gussuri/codex-reset-observatory) 汇总的未来 12/24/48/72 小时社区概率；不是 OpenAI 官方数据，也不参与自动续跑。 | 无需凭据，可在设置中隐藏。 |
...
重置预测来自 MIT License 开源项目 [Codex Reset Observatory](https://github.com/gussuri/codex-reset-observatory) 的公共接口。上游项目综合历史重置、状态与社区信号估算概率；本项目只读取并展示其 12/24/48/72 小时结果，不训练预测模型，也不把预测结果用于自动续跑。
...
- “重置预测”只向 README 中公开说明的 Codex Reset Observatory 固定 HTTPS 接口发送无凭据 GET 请求，不上传任何本机用量或对话数据。
...
8. **重置预测指示灯变红**：表示 Codex Reset Observatory 返回数据过期或本轮请求失败；保留的概率仍会显示。它与官方订阅额度无关，等待下一次 5 分钟刷新即可。
