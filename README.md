...
如果不设置，默认按 `0` 开始。设置初始值是因为第三方日志接口通常只返回有限页数，监视器会记住初始值，并把之后新发现的日志 Token 累加到本地。程序会保存每个日志 ID 上次读取到的 Token 数；同一日志后续结算发生变化时只累加差值，避免漏算或重复。同一个状态文件还会按本机日期持久化“今日 Token”，Codex 或 Windows 重启后不会清空，第二天自动归零。该状态只保存在 `%LOCALAPPDATA%\CodexUsageMonitor\account-token-counter.json`，不会进入仓库。

### 1.3 展开监视栏查看和勾选

点击监视栏即可展开三栏面板。每项前的复选框决定是否显示在折叠后的监视栏中，最多显示 8 项；三栏共用 30 秒刷新周期。请求失败时保留上一次成功数据，指示灯变为灰色，不会把已有数值清空。

面板左下方提供两个显示开关：开启“极简模式”后，折叠监视栏只显示数值与单位，不显示数据项名称，可最多选择 14 项，并在选中 9 项及以上时启用双行压缩布局；普通模式仍最多选择 8 项。开启“倒计时可视化”后，折叠监视栏最左侧显示与文字同色的圆形表盘，12 点方向有向外凸起的小块，绿色指针每 30 秒顺时针旋转一圈，刷新后重新从 12 点开始。这些选择保存在 Codex 本地页面设置中。
...
| 今日 Token | 从本机当天 00:00 起累计输入 Token 与输出 Token；按日志 ID 保存上次值并补记后续结算差值，按日期持久化，重启后继续累加。 |
| 累计 Token | 初始值加上之后新日志及同一日志后续增长的 Token 差值；日志分页有限，因此需要初始值。 |
...
额度默认保留一位小数，单条请求的消耗额度保留三位。API 账户每 30 秒固定刷新日志第一页，同时在其余历史页中循环刷新一页；若共有 10 页，约 4.5 分钟完成一次历史页校准。接口只返回有限日志时，界面会保留成功数据并提示汇总范围。
...
API Key 接口返回 `HTTP 429` 时，该栏会显示“请求受限”并保留上一次成功数据。监视器会跳过后续的立即请求，从 60 秒开始逐步延长重试间隔，最长 5 分钟；请求恢复成功后自动回到 30 秒刷新。单实例与退避修复可以避免本机重复请求放大限流，但无法阻止服务商自身的临时限流；如果服务商未返回 `Retry-After` 或 `X-RateLimit-*` 响应头，监视器无法显示其真实限流窗口。
