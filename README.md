...
- API 账户：如需启用，请准备服务商的 Base URL、用户 ID 和账户访问令牌。访问令牌一般可在服务商的“用户资料”“个人中心”或类似页面获取。
- API Key：如需启用，请准备 API 密钥；不同服务商的用量接口和返回字段可能不同，建议同时准备相关接口文档，交给 Codex 自动适配。
- 安全软件：项目使用隐藏 PowerShell、Node.js 后台进程和 CDP 参数，可能触发安全软件的启发式误报。建议保留防护；确认安装包来自本仓库后，仅为项目目录、启动脚本或相关进程添加精确信任规则，不要长期关闭整套杀毒软件。
