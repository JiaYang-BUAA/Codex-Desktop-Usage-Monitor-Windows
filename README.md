...
**v1.8.4 更新：**审批模式切换时监视栏持续可见；重置时间显示具体月日和分钟；启动器优先使用独立 Codex CLI，避免不兼容的桌面捆绑 CLI 查询官方额度时持续超时；同步增强 Windows PowerShell 兼容性。
...
面板右下角、“Codex Usage Monitor for Windows v1.8.4”标识上方提供两个显示开关以及“最多显示 · 刷新”提示：开启“极简模式”后，折叠监视栏只显示数值与单位，不显示数据项名称，可最多选择 14 项，并在选中 9 项及以上时启用双行压缩布局；普通模式仍最多选择 8 项。开启“倒计时可视化”后，折叠监视栏最左侧显示与文字同色的圆形表盘，绿色指针每 60 秒顺时针旋转一圈。这些选择保存在 Codex 本地页面设置中。
