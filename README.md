...
**v1.8.5 更新：**修复 Codex 更新后快捷方式图标失效、任务区数据暂缺时消失和渲染器重载后监视进程过早退出的问题；同时加固凭据传输、CDP 连接、API 日志分页、发布清单及依赖供应链。
...
面板右下角、“Codex Usage Monitor for Windows v1.8.5”标识上方提供两个显示开关以及“最多显示 · 刷新”提示：开启“极简模式”后，折叠监视栏只显示数值与单位，不显示数据项名称，可最多选择 14 项，并在选中 9 项及以上时启用双行压缩布局；普通模式仍最多选择 8 项。开启“倒计时可视化”后，折叠监视栏最左侧显示与文字同色的圆形表盘，绿色指针每 60 秒顺时针旋转一圈。这些选择保存在 Codex 本地页面设置中。
