...
**v1.8.6 更新：**修复正式 Release 包缺少仓库专用 `.github` 目录时内置测试中断的问题；源码检出继续严格校验发布工作流，并新增从构建产物直接运行包内自测的回归覆盖，确保正式包可以独立验证。
...
面板右下角、“Codex Usage Monitor for Windows v1.8.6”标识上方提供两个显示开关以及“最多显示 · 刷新”提示：开启“极简模式”后，折叠监视栏只显示数值与单位，不显示数据项名称，可最多选择 14 项，并在选中 9 项及以上时启用双行压缩布局；普通模式仍最多选择 8 项。开启“倒计时可视化”后，折叠监视栏最左侧显示与文字同色的圆形表盘，绿色指针每 60 秒顺时针旋转一圈。这些选择保存在 Codex 本地页面设置中。
